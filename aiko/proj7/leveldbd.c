...
#define USAGE_MESSAGE "Usage: %s -v -s _l0_size_ _directory_ _virtual_disk_name_\n"
...
void * mmap_level0_file(struct LevelDB_G * ldb, char * filename) {
	...
	assert( stat_struct.st_size==(ldb->l0_n*KV_PAIR_SIZE)) ;
...
int open_level1_file(struct LevelDB_G * ldb, char * filename) {
	char * path ;
	
	path = (char *)malloc(strlen(ldb->backing_dir)+strlen(filename)+2) ;
	strcat(strcat(strcat(path,ldb->backing_dir),"/"),filename) ;
	
	ldb->l1_id = open(path, O_RDWR |  O_CREAT, 0664 ) ;
	free(path) ;
...
int main(int argc, char * argv[]) {
	...
	if ( leveldb_g.l0_n <=8 ) leveldb_g.l0_n = LEVEL0_N_PAIRS ;
	
	
	assert(mmap_level0_file(&leveldb_g, LEVEL0_NAME)) ;
	assert(!open_level1_file(&leveldb_g, LEVEL1_NAME)) ;
